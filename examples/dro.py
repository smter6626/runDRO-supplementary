...
with the updates
    ...
    theta  <- theta  + eta_theta  * grad_theta  log(E_Q[P])
...
ETA_THETA = 1e-2   # learning rate for theta (P, ascent on log E)
...
for p_iter in range(1, NUM_P_ITERS + 1):
    ...
    # --- one outer iteration: update theta (P), ascent on log(E_Q[P]) ---
    log_exp_val, grad_theta, _ = log_exp_query_and_grad(P_theta, Q_phi)
